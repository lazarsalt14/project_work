...
std::vector<double> random_phases(size_t N, std::mt19937 &g) {
    ...
    for (size_t i = 0; i < N; ++i) th[i] = dist(g);
    return th;
}

// --- ifftshift helper for std::complex<double> ---
void ifftshift(std::vector<cd>& v) {
    size_t N = v.size();
    size_t half = N / 2;
    if (N % 2 == 0)
        std::rotate(v.begin(), v.begin() + half, v.end());
    else
        std::rotate(v.begin(), v.begin() + half + 1, v.end());
}
                    ...
                    double h_init, double h_min, double h_max)
{
    ...
    while(x < x1){
        ...
        double fac = 0.9*std::pow(err + 1e-16,-0.2);
                                             ...
                                             std::mt19937 &g)
{
    ...
    double ds = data.ds;
    size_t Nsteps = static_cast<size_t>(std::llround(FL/ds));

    std::vector<double> z0;
    double dz = ds/10.0;
    for(double z=dz; z<=FL+1e-12; z += dz) z0.push_back(z);
    size_t L2 = z0.size();

    // --- Generate U0 using FFTW like MATLAB ---
    std::vector<double> th1 = random_phases(L2, g);
    std::vector<cd> freq(L2);
    for (size_t i = 0; i < L2; ++i)
        freq[i] = std::polar(1.0, th1[i]);

    ifftshift(freq);

    fftw_complex* in  = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*L2);
    fftw_complex* out = (fftw_complex*) fftw_malloc(sizeof(fftw_complex)*L2);
    for (size_t i=0; i<L2; ++i) {
        in[i][0] = freq[i].real();
        in[i][1] = freq[i].imag();
    }

    fftw_plan p = fftw_plan_dft_1d((int)L2, in, out, FFTW_BACKWARD, FFTW_ESTIMATE);
    fftw_execute(p);
    fftw_destroy_plan(p);

    std::vector<cd> time(L2);
    for (size_t i=0; i<L2; ++i)
        time[i] = cd(out[i][0], out[i][1]) / (double)L2;
    ifftshift(time);

    double scale = sqrt(10.0 * (double)L2);
    std::vector<cd> U0(L2);
    for (size_t i=0; i<L2; ++i)
        U0[i] = time[i] * scale;
    ...
    for(size_t i=0;i<=Nsteps;++i){
        ...
        if(i < Nsteps){
            double zstart = double(i) * ds;
            double zend   = double(i+1) * ds;
            auto sol = rk45_dopri([&](double zz,const State& yy){ return ode(zz,yy); },
                                   zstart, zend, y, 1e-6, 1e-6, 1e-3, 1e-8, ds/2.0);
            y = sol.y.back();
        }
    }
    return powers;
...
{
    size_t Nsteps = static_cast<size_t>(std::llround(data.fiberLength / data.ds));
    std::vector<std::array<double,4>> avg(Nsteps+1);
    std::vector<std::array<double,4>> avg2(Nsteps+1);
    for(auto &r : avg) r.fill(0.0);
    for(auto &r : avg2) r.fill(0.0);
    ...
    for(int run=0; run<data.avg_of; ++run){
        futures.push_back(std::async(std::launch::async, [&, run](){
            std::mt19937 g(run+1);
            return run_single(data, Bending_radius, launch_power, kappa, g);
        }));
    }
    ...
    for(size_t i=0;i<=Nsteps;++i)
        for(int c=0;c<4;++c)
            avg[i][c] /= double(data.avg_of);

    std::vector<std::array<double,4>> stddev(Nsteps+1);
    for(size_t i=0;i<=Nsteps;++i)
        for(int c=0;c<4;++c)
            stddev[i][c] = (data.avg_of>1) ?
                std::sqrt(avg2[i][c]/double(data.avg_of)-avg[i][c]*avg[i][c]) : 0.0;
...
int main(){
    ...
    std::vector<std::vector<double>> kappa = {
        {0, 0.00894840159268594, 1.32706128892888e-05, 0.00867937437683011},
        {0.00904060792822696, 0, 0.00888451580613146, 1.27735284247076e-05},
        {1.36450819178854e-05, 0.00894200470685365, 0, 0.00860305054846481},
        {0.00898870850362288, 1.32555945413147e-05, 0.00876623363739193, 0}
    };

    for (int power=1; power<=4; ++power) {
        auto t_start = std::chrono::high_resolution_clock::now();
        std::cout << "Simulating launch_power = " << power << "...\n";
        core_power_hetro_avg(data, Bending_radius, power, kappa, "Hetro_PL_core_cpp_fftw");
